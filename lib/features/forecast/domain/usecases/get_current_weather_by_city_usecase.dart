import 'package:dartz/dartz.dart';
import 'package:injectable/injectable.dart';
import 'package:weather_around_app/core/domain/usecases/usecase.dart';
import 'package:weather_around_app/core/error/failure.dart';
import 'package:weather_around_app/features/forecast/data/repositories/forecast_repository.dart';
import 'package:weather_around_app/features/forecast/domain/entities/forecast_card/current_weather_city_entity.dart';
import 'package:weather_around_app/features/forecast/domain/entities/params/get_current_weather_by_city_params.dart';
import 'package:weather_around_app/features/forecast/domain/mapper/forecast_card/forecast_card_mapper.dart';

@injectable
class GetCurrentWeatherByCityUseCase
    extends UseCase<CurrentWeatherCityEntity, GetCurrentWeatherByCityParams> {
  final ForecastRepository _forecastRepository;

  GetCurrentWeatherByCityUseCase(this._forecastRepository);
  @override
  Future<Either<Failure, CurrentWeatherCityEntity>> call(params) async {
    final currentWeatherResult =
        await _forecastRepository.getCurrentWeatherConditionByCityRepository(
      params.latitudeLongitude,
    );

    return currentWeatherResult.fold(
      (failure) => Left(failure),
      (currentWeatherModel) {
        return Right(
          ForecastMapper.fromCurrentWeatherModelToEntity(
            currentWeatherModel,
          ),
        );
      },
    );
  }
}
